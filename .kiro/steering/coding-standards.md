...
```javascript
try {
	const { data, error } = await supabase.from('entities').select('*').eq('campaign_id', campaignId);
...
[
	'entityIndex',
	campaignId,
] // Smart-text entity index
[
	('entities', campaignId, type)
] // Entity list by type
[
	('entry', campaignId, entityId)
] // Wiki detail page
[
	('timeline', campaignId)
] // Timeline data
