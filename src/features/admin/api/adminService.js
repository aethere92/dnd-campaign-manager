...
const buildMapPayload = (data, jsonStorage) => {
	// 1. Construct the Config Object (Nested)
	const config = {
		path: jsonStorage.path,
		parentId: jsonStorage.parentId || null, // null if empty string
		fileExtension: jsonStorage.fileExtension || 'webp',
		mapId: jsonStorage.key || data.name?.toLowerCase().replace(/\s+/g, '_'),
		sizes: {
			maxZoom: parseInt(jsonStorage.maxZoom || 4),
			imageWidth: parseInt(jsonStorage.imageWidth || 0),
			imageHeight: parseInt(jsonStorage.imageHeight || 0),
		},
	};

	// 2. Construct the Data Object (Skeleton)
	// Only set this on CREATE, not UPDATE (unless you want to overwrite data)
	const initialData = {
		areas: {},
		paths: [],
		overlays: [],
		...
		if (jsonSource.sizes) {
			['imageWidth', 'imageHeight', 'maxZoom'].forEach((k) => {
				if (jsonSource.sizes[k] !== undefined) {
					attributesList.push({ name: k, value: jsonSource.sizes[k] });
				}
			});
		}
	} else {
...
export const fetchMapKeys = async (campaignId) => {
	const { data, error } = await supabase.from('maps').select('key').eq('campaign_id', campaignId).order('key');
