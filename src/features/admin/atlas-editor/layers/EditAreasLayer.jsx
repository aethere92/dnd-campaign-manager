// --- FILE: features/admin/atlas-editor/layers/EditAreasLayer.jsx ---
...
const AreaItem = React.memo(
	({ area, isSelected, isInteractive, actions }) => {
		...
		// Styles
		// Depends on `area` as a whole: getAreaPatternId reads the whole object, so
		// listing individual fields would under-specify what this actually reads
		// (the React Compiler flags the mismatch). `area` is a stable reference from
		// the reducer between edits, so this doesn't recompute needlessly.
		const fillStyle = useMemo(() => {
			let fillColor = area.interiorColor || '#ff0000';
			let fillOpacity = area.fillOpacity ?? 0.2;

			// Shared id builder, so the editor references exactly the pattern PatternDefs
			// registers (previously the id string was inlined and could drift).
