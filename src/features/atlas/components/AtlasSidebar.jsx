...
import { useAtlasSearch } from '@/features/atlas/hooks/useAtlasSearch';
import { SidebarItem } from './sidebar/SidebarItem';
import { SidebarGroup } from './sidebar/SidebarGroup';

// Hoisted out of AtlasSidebar's render body: a component declared during render is
// a new type each pass, so React remounts it. activeTab/onSelect are now props.
const Tab = ({ id, icon: Icon, label, activeTab, onSelect }) => (
	<button
		onClick={() => onSelect(id)}
		className={clsx(
		...
		<>
			...
			<div
				...
				<div className='flex border-b border-border bg-muted/50 shrink-0'>
					<Tab id='locations' icon={LocateFixed} label='Places' activeTab={activeTab} onSelect={setActiveTab} />
					<Tab id='journal' icon={BookOpen} label='Journal' activeTab={activeTab} onSelect={setActiveTab} />
					<Tab id='layers' icon={Layers} label='Layers' activeTab={activeTab} onSelect={setActiveTab} />
				</div>
