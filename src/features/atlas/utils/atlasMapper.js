...
/**
 * Coerce a map's stored data — which may be a JSON string or an already-parsed
 * object — into a plain object, falling back to {} on malformed JSON.
 *
 * Both the tactical-map editor and the wiki mini-map read map data that can arrive
 * in either shape; this was the same try/JSON.parse/catch block copied verbatim in
 * both.
 */
export const parseMapData = (raw) => {
