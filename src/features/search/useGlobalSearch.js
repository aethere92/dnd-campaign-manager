import { useState, useEffect, useMemo, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { useQuery } from '@tanstack/react-query';
import { useCampaign } from '@/features/campaign/CampaignContext';
import { globalSearch } from '@/features/search/api/searchService';
import { aiSearch, isAiSearchCoolingDown } from '@/features/search/api/aiSearchService';
import { getEntityConfig } from '@/domain/entity/config/entityConfig';
import { transformSearchResults } from '@/features/search/utils/searchMapper';
import { useSearch } from './SearchContext';

const STORAGE_KEY = 'recent-searches';

export function useGlobalSearchViewModel() {
	const navigate = useNavigate();
	const { campaignId } = useCampaign();

	const { isOpen, openSearch: setIsOpen, closeSearch, query, setQuery, aiMode, toggleAiMode } = useSearch();

	const [selectedIndex, setSelectedIndex] = useState(0);
	const [aiAnswer, setAiAnswer] = useState(null);
	const [aiLoading, setAiLoading] = useState(false);
	const [aiError, setAiError] = useState(null);
	const [recentSearches, setRecentSearches] = useState(() => {
		try {
			const stored = localStorage.getItem(STORAGE_KEY);
			return stored ? JSON.parse(stored) : [];
		} catch {
			return [];
		}
	});

	// Keyword search (always runs on query change)
	const { data: rawData, isLoading: keywordLoading } = useQuery({
		queryKey: ['globalSearch', campaignId, query],
		queryFn: () => globalSearch(campaignId, query),
		enabled: !!campaignId && query.trim().length > 0,
		staleTime: 1000 * 30,
	});

	// Clear AI answer when query changes (user is typing a new query)
	useEffect(() => {
		setAiAnswer(null);
		setAiError(null);
	}, [query]);

	// AI search — triggered explicitly via submitAiSearch
	const submitAiSearch = useCallback(async () => {
		if (!aiMode || !campaignId || query.trim().length < 2) return;

		if (isAiSearchCoolingDown()) {
			setAiError('AI search temporarily unavailable (rate limited). Showing keyword results.');
			return;
		}

		setAiLoading(true);
		setAiError(null);
		setAiAnswer(null);

		try {
			const result = await aiSearch(campaignId, query);
			setAiAnswer(result);
		} catch (err) {
			console.error('[AI Search]', err.message);
			setAiAnswer(null);
			setAiError(
				err.message.includes('Rate limited')
					? 'AI rate limited — showing keyword results for 60s.'
					: 'AI search failed — showing keyword results.'
			);
		} finally {
			setAiLoading(false);
		}
	}, [aiMode, campaignId, query]);

	// Transform keyword results
	const results = useMemo(() => {
		if (!rawData) return [];
		const processedItems = transformSearchResults(rawData);

		return processedItems.map((result) => {
			const config = getEntityConfig(result.type);
			return {
				id: result.id,
				name: result.name,
				type: result.type,
				typeLabel: result.type.toUpperCase(),
				description: result.description || '',
				icon: config.icon,
				theme: config.tailwind,
			};
		});
	}, [rawData]);

	const flattenedRawResults = useMemo(() => {
		if (!rawData) return [];
		return transformSearchResults(rawData);
	}, [rawData]);

	const recentSearchesViewModel = useMemo(() => {
		return recentSearches.map((result) => {
			const config = getEntityConfig(result.type);
			return {
				id: result.id,
				name: result.name,
				type: result.type,
				typeLabel: result.type.toUpperCase(),
				description: result.description || '',
				icon: config.icon,
				theme: config.tailwind,
			};
		});
	}, [recentSearches]);

	// Determine what to show
	const isLoading = aiMode ? aiLoading : keywordLoading;
	const showAiAnswer = aiMode && aiAnswer && !aiError;
	const showKeywordResults = !aiMode || !aiAnswer || aiError;

	const handleSelect = (result) => {
		const rawResult = flattenedRawResults.find((r) => r.id === result.id);

		if (rawResult) {
			const toStore = {
				id: rawResult.id,
				name: rawResult.name,
				type: rawResult.type,
				description: rawResult.description || '',
			};
			const updated = [toStore, ...recentSearches.filter((r) => r.id !== toStore.id)].slice(0, 5);
			setRecentSearches(updated);
			try {
				localStorage.setItem(STORAGE_KEY, JSON.stringify(updated));
			} catch {}
		}

		navigate(`/wiki/${result.type}/${result.id}`);
		closeSearch();
		setSelectedIndex(0);
	};

	const clearRecent = () => {
		setRecentSearches([]);
		try {
			localStorage.removeItem(STORAGE_KEY);
		} catch {}
	};

	// Arrow key navigation + Enter to submit AI search
	useEffect(() => {
		if (!isOpen) return;

		const handleKeyDown = (e) => {
			if (e.key === 'Enter') {
				e.preventDefault();
				if (aiMode && !aiAnswer && !aiLoading) {
					submitAiSearch();
				} else if (results[selectedIndex]) {
					handleSelect(results[selectedIndex]);
				}
				return;
			}
			if (results.length === 0) return;
			if (e.key === 'ArrowDown') {
				e.preventDefault();
				setSelectedIndex((prev) => (prev + 1) % results.length);
			} else if (e.key === 'ArrowUp') {
				e.preventDefault();
				setSelectedIndex((prev) => (prev - 1 + results.length) % results.length);
			}
		};

		window.addEventListener('keydown', handleKeyDown);
		return () => window.removeEventListener('keydown', handleKeyDown);
	}, [isOpen, results, selectedIndex, aiMode, aiAnswer, aiLoading, submitAiSearch]);

	return {
		isOpen,
		setIsOpen,
		closeSearch,
		query,
		setQuery,
		results,
		isLoading,
		recentSearches: recentSearchesViewModel,
		selectedIndex,
		setSelectedIndex,
		handleSelect,
		clearRecent,
		// AI-specific
		aiMode,
		toggleAiMode,
		aiAnswer,
		aiLoading,
		aiError,
		showAiAnswer,
		showKeywordResults,
		submitAiSearch,
	};
}
