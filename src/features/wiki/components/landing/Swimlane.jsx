...
export const Swimlane = ({ title, parentTitle, link, items, config, context }) => {
	...
	return (
		<div className='mb-8 last:mb-0 animate-in fade-in slide-in-from-bottom-3 duration-500'>
			...
			<div className='flex items-center gap-2 border-b border-border/60 pb-2 mb-4 group/header'>
				{parentTitle && (
				...
				<h2 className='text-sm font-bold text-foreground flex items-center gap-2 uppercase tracking-wide'>
					...
					{link && (
						<Link
							to={link}
							className='text-muted-foreground/30 hover:text-accent transition-all hover:translate-x-0.5'
							title={`View ${title}`}>
							<ArrowRight size={12} />
						</Link>
					)}
				...
				<span className='text-[9px] font-bold bg-muted/50 px-2 py-0.5 rounded-full text-muted-foreground/60 ml-auto border border-border/50'>
					{items.length}
				</span>
			</div>
